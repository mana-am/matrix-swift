...
/// 3×3 column wave (left → right); `dmx-ripple-3` keyframe, per-dot delay =
/// colPath ×0.19 ×cycle (`dmx-path-3`, dur ×0.68). Mirrors `dotm-3x3-7.tsx`.
public struct Dotm3x3_7: View {
    var props = DotMatrixCommonProps(speed: 1.75, pattern: .full)
    init(props: DotMatrixCommonProps) { self.props = props }

    /// Source-parity ergonomic init — mirrors `<Dotm3x3_7 size=… />` upstream. The
    ...
    public var body: some View {
        DotMatrix3Base(props: props, bypassOpacityRemap: true) { ctx, now in
            ...
            let path = DotMatrix3GridPaths.colWaveNorm(ctx.col)
